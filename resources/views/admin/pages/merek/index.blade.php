    ...
    <div class="col-md-12">
      <div class="card shadow">
        <div class="card-header d-flex justify-content-between align-items-center">
          ...
          <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addMerek">
            <i class="bi bi-plus-circle me-1"></i>
            Tambah
          </button>
          @include('admin.pages.merek.create')
        </div>
                ...
                @foreach ($mereks as $merek)
                  <tr>
                    ...
                    <td class="text-nowrap">
                      <button type="button" class="btn btn-sm btn-success mx-1" data-bs-toggle="modal"
                      ...
                      <form id="data-{{ $merek->id }}" action="{{ route('merek.destroy', $merek->id) }}" method="post"
                        class="d-inline">
                        @csrf
                        @method('delete')
                        <button class="btn btn-danger btn-sm mx-1" type="submit"
                          onclick="event.preventDefault(); confirmDelete({{ $merek->id }})">
                          <i class="bi bi-trash-fill"></i>
                        </button>
                      </form>
                    </td>
                  </tr>
                  @include('admin.pages.merek.edit')
                  @include('admin.pages.merek.show')
                @endforeach
              </tbody>
            </table>
          </div>
